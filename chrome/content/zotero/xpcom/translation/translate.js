	...
	"resolveURL": function (url, dontUseProxy) {
		...
		if (!m) {
			...
			if (Zotero.isFx && this.location) {
				resolved = Components.classes["@mozilla.org/network/io-service;1"].getService(Components.interfaces.nsIIOService).newURI(this.location, "", null).resolve(url);
		...
		return this._itemSaver.saveItems(items.slice(), attachmentCallback.bind(this),
			...
			.then(function (newItems) {
				...
				for (let item of this.newItems) {
					dump('saved and done: ' + item.getField('title') + "\n");
				}
			}.bind(this))
			.catch((e) => {
				this._savingItems -= items.length;
...
Zotero.Translate.Import.prototype._prepareTranslation = Zotero.Promise.method(function () {
	...
	if (this.location) {
		try {
			baseURI = Components.classes["@mozilla.org/network/io-service;1"].getService(Components.interfaces.nsIIOService).newFileURI(this.location);
		} catch (e) {
		}
	}
